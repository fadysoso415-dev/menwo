...
import { X, Mail, Lock, User as UserIcon, Shield, Eye, EyeOff, Sparkles, Trophy, Coins, ArrowLeft, Phone, ShieldCheck, Loader2 } from 'lucide-react';
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (gUser && gUser.email) {
        const existingUser = allUsers.find(u => u.email.toLowerCase() === gUser.email?.toLowerCase());
        if (existingUser) {
          const userToLogin = existingUser.email.toLowerCase() === 'fadysoso415@gmail.com'
            ? { ...existingUser, isAdmin: true }
            : existingUser;
          onLoginSuccess(userToLogin);
          onClose();
        } else {
          const isUserAdmin = gUser.email.toLowerCase() === 'fadysoso415@gmail.com' || gUser.email.toLowerCase().includes('admin');
          const newUser: User = {
            ...
            balance: 0,
            isAdmin: isUserAdmin,
            avatar: gUser.photoURL || AVATAR_OPTIONS[0].url,
      ...
      if (err?.code === 'auth/popup-closed-by-user') {
        setError('تم إغلاق نافذة تسجيل الدخول من جوجل قبل إكمال العملية.');
      } else {
        setError(err?.message || 'تعذر تسجيل الدخول بواسطة جوجل. حاول مجدداً.');
      }
    } finally {
      ...
      >
        ...
        <div className="mb-5 relative z-10">
          <button
            ...
            className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-2xl bg-zinc-900 hover:bg-zinc-800 border border-zinc-700/80 hover:border-zinc-600 text-white font-black text-xs sm:text-sm transition-all shadow-lg active:scale-98 disabled:opacity-60 cursor-pointer group"
          ...
          >
            ...
            <span>{isLoadingGoogle ? 'جاري الاتصال بحساب جوجل...' : 'المتابعة بضغطة واحدة عبر Google'}</span>
          </button>

          <div className="relative flex items-center justify-center my-4">
            <div className="border-t border-zinc-800/80 w-full"></div>
            <span className="bg-zinc-950 px-3 text-[11px] font-bold text-zinc-500 absolute">أو بالبريد الإلكتروني</span>
          </div>
        </div>
